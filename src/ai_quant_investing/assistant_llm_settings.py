...
def _validate_hosted_provider(provider: str) -> str:
    if (provider or "").strip().lower() in {"deterministic", "mock", "demo", "offline"}:
        raise ValueError("Workbench Assistant supports hosted LLM providers only.")
    normalized = normalize_provider(provider)
    if display_provider(normalized) not in ASSISTANT_PROVIDER_LABELS:
        valid = ", ".join(ASSISTANT_PROVIDER_LABELS)
        raise ValueError(f"Unsupported Workbench Assistant provider '{provider}'. Supported providers: {valid}.")
    return normalized
    ...
    *,
    ...
    if not api_key:
        warnings.append(
            ...
            f"or set {env_name}; the Workbench Assistant will wait for a real LLM key."
        )
