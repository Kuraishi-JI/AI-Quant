...
def walk_forward_predictions(panel: pd.DataFrame, config: PrototypeConfig) -> tuple[pd.DataFrame, dict[pd.Timestamp, Pipeline], Pipeline]:
    ...
    if len(unique_dates) <= min_train_days + 5:
        raise ValueError("Not enough history to run the walk-forward prototype.")
...
def generate_signal_weights(
    ...
    """Translate predictions into FinRL-X style target weights plus risk overlays."""
...
def run_prototype(prices: pd.DataFrame, config: PrototypeConfig) -> PrototypeResult:
